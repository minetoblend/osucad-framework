...
export class ScreenStack extends CompositeDrawable {
  ...
  #suspend(from: IScreen | null, to: IScreen) {
    ...
    if (from.isLoaded) performSuspend();
    else {
  ...
  makeCurrent(target: IScreen) {
    ...
    if (!this.#stack.includes(target)) throw new Error(`Screen not in stack: ${target.name}`);
    ...
    while (this.currentScreen !== null) {
      ...
      if (exitBlocked) {
        ...
        if (this.currentScreen === exitCandidate) this.#resumeFrom(exitSource);
  ...
  #exitFrom(
    ...
    if (shouldFireExitEvent && toExit.isLoaded) {
      ...
      if ((source === null || toExit.validForResume) && blockRequested) return true;
    ...
    if (shouldFireResumeEvent) this.#resumeFrom(toExit);
  ...
  override updateChildrenLife(): boolean {
    if (!super.updateChildrenLife()) return false;
