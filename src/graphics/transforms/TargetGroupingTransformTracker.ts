...
export class TargetGroupingTransformTracker {
  ...
  #lastAppliedTransformIndices = new Map<string, number>();
  ...
  updateTransforms(time: number, rewinding: boolean) {
    ...
    for (let i = this.#getLastAppliedIndex(); i < this.#transforms.length; ++i) {
      let t = this.#transforms.get(i)!;

      const tCanRewind = !this.transformable.removeCompletedTransforms && t.rewindable;

      let flushAppliedCache = false;

      if (time < t.startTime) break;
      ...
      if (!t.appliedToEnd) {
        ...
        if (t.appliedToEnd) {
          ...
          if (t.isLooping) {
            ...
            if (tCanRewind) t.loopCount = oldLoopCount;
      ...
      if (flushAppliedCache) this.#resetLastAppliedCache();
      // if this transform is applied to end, we can be sure that all previous transforms have been completed.
      else if (t.appliedToEnd) this.#setLastAppliedIndex(t.targetMember, i + 1);
    }
  ...
  finishTransforms(targetMember?: string) {
    ...
    if (targetMember == null) toFlushPredicate = (t) => !t.isLooping;
    else toFlushPredicate = (t) => !t.isLooping && t.targetMember === targetMember;
  ...
  #getLastAppliedIndex(targetMember?: string) {
    if (targetMember == null) {
      ...
      for (const i of this.#lastAppliedTransformIndices.values()) {
        if (i < min) min = i;
      }
    ...
    return this.#lastAppliedTransformIndices.get(targetMember) ?? 0;
  }

  #setLastAppliedIndex(targetMember: string, index: number) {
    this.#lastAppliedTransformIndices.set(targetMember, index);
  }

  #resetLastAppliedCache() {
    for (const tracked of this.#targetMembers) this.#lastAppliedTransformIndices.set(tracked, 0);
  }
}
