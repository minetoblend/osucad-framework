...
export namespace EasingFunction {
  ...
  export const InExpo: EasingFunction = (time) => Math.pow(2, 10 * (time - 1)) + expo_offset * (time - 1);
  export const OutExpo: EasingFunction = (time) => -Math.pow(2, -10 * time) + 1 + expo_offset * time;
  export const InOutExpo: EasingFunction = (time) => {
    if (time < 0.5) {
      return (Math.pow(2, 20 * time - 10) + expo_offset * (10 * time - 5)) / 2;
    } else {
      return (2 - Math.pow(2, -20 * time + 10) - expo_offset * (10 * time - 5)) / 2;
    }
  };
  ...
  export const InElastic: EasingFunction = (time) =>
    -Math.pow(2, -10 + 10 * time) * Math.sin((1 - elastic_const2 - time) * elastic_const) +
    elastic_offset_full * (1 - time);
  export const OutElastic: EasingFunction = (time) =>
    Math.pow(2, -10 * time) * Math.sin((time - elastic_const2) * elastic_const) + 1 - elastic_offset_full * time;
  ...
  export const InBounce: EasingFunction = (time) => {
    time = 1 - time;
    if (time < bounce_const) return 1 - 7.5625 * time * time;
    if (time < 2 * bounce_const) return 1 - (7.5625 * (time -= 1.5 * bounce_const) * time + 0.75);
    if (time < 2.5 * bounce_const) return 1 - (7.5625 * (time -= 2.25 * bounce_const) * time + 0.9375);
  ...
  export const OutPow10: EasingFunction = (time) => --time * Math.pow(time, 10) + 1;
}
