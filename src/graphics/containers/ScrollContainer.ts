...
export abstract class ScrollContainer extends Container {
  ...
  override onMouseDown(e: MouseDownEvent): boolean {
    if (this.isDragging || e.button != MouseButton.Left) return false;
  ...
  override onDrag(e: DragEvent): boolean {
    ...
    const decay = Math.pow(0.95, timeDelta);
  ...
  override onDragEnd(e: DragEndEvent): boolean {
    ...
    if (this.#averageDragTime <= 0.0) return false;

    let velocity = this.#averageDragDelta / this.#averageDragTime;

    const velocity_cutoff = 0.1;
    if (Math.abs(Math.pow(0.95, this.time.current - this.#lastDragTime) * velocity) < velocity_cutoff) {
      velocity = 0;
    }
  ...
  override onScroll(e: ScrollEvent): boolean {
    if (this.content['aliveInternalChildren'].length === 0) return false;
  ...
  scrollIntoView(d: Drawable, animated = true) {
    ...
    else if (maxPos > this.current + this.displayableContent) this.scrollTo(maxPos - this.displayableContent, animated);
  }
  ...
  #toScrollbarPosition(scrollPosition: number): number {
    if (almostEquals(0, this.scrollableExtent)) return 0;
  ...
  #fromScrollbarPosition(scrollbarPosition: number): number {
    if (almostEquals(0, this.scrollbarMovementExtent)) return 0;
