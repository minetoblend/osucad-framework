...
import { CompositeDrawable, Container, FillDirection, FillFlowContainer, ScrollContainer } from '../containers';
import {
  Anchor,
  Axes,
  Direction,
...
export abstract class Menu extends CompositeDrawable {
  ...
  constructor(direction: Direction, topLevelMenu: boolean = false) {
    ...
    if (topLevelMenu) this.#state = MenuState.Open;
  ...
  set maxWidth(value: number) {
    if (almostEquals(this.#maxWidth, value)) return;
  ...
  set maxHeight(value: number) {
    if (almostEquals(this.#maxHeight, value)) return;
  ...
  #menuItemClicked = (item: DrawableMenuItem) => {
    ...
    if (item.item.items.length === 0) {
      ...
      if (item.closeMenuOnClick) this.#closeAll();
  ...
  override onFocusLost(e: FocusLostEvent): boolean {
    if (this.#submenu?.state === MenuState.Open) return false;
  ...
  closeFromChild(source: MenuItem) {
    if (this.isHovered || (this.#parentMenu?.isHovered ?? false)) return;
...
export abstract class DrawableMenuItem extends CompositeDrawable {
  ...
  override onClick(): boolean {
    ...
    if (!this.isActionable) return true;
