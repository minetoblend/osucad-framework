...
import { InputKey, compareInputKeys, isPhysical, isVirtual } from '../state/InputKey';
...
export class KeyCombination {
  ...
  isPressed(pressedKeys: KeyCombination, inputState: InputState, matchingMode: KeyCombinationMatchingMode): boolean {
    ...
    if (arrayEquals(this.keys, KeyCombination.none)) return false;
  ...
  static fromScrollDelta(delta: Vec2): InputKey[] {
    const keys: InputKey[] = [];
    if (delta.y > 0) keys.push(InputKey.MouseWheelUp);
    if (delta.y < 0) keys.push(InputKey.MouseWheelDown);
    if (delta.x > 0) keys.push(InputKey.MouseWheelRight);
    if (delta.x < 0) keys.push(InputKey.MouseWheelLeft);
    return keys;
  }
  ...
  static keyBindingContains(candidateKeyBinding: readonly InputKey[], physicalKey: InputKey): boolean {
    if (candidateKeyBinding.includes(physicalKey)) return true;
