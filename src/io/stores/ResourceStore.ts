...
export class ResourceStore<T> implements IResourceStore<T> {
  ...
  bindReload(name: string, onReload: () => void) {
    if (this.#actionList.has(name)) throw new Error(`A reload delegate is already bound to the resource '${name}'`);
  ...
  addExtension(extension: string) {
    ...
    if (!this.#searchExtensions.includes(extension)) this.#searchExtensions.push(extension);
  }
